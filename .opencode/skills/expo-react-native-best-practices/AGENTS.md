...
> generating, or refactoring React Native and Next.js codebases. Humans  
...
Comprehensive performance optimization guide for React Native and Next.js applications, designed for AI agents and LLMs. Contains 40+ rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.9 [Per-Request Deduplication with React Native.cache()](#39-per-request-deduplication-with-reactcache)
...
6. [Rendering Performance](#6-rendering-performance) — **MEDIUM**
   ...
   - 6.10 [Use React Native Render Tree Resource Hints](#610-use-react-dom-resource-hints)
...
This matters when `getFlag` hits the network, a feature-flag service, or `React Native.cache` / DB work: skipping it when `someCondition` is false removes that cost on the cold path.
...
Popular icon and component libraries can have **up to 10,000 re-exports** in their entry file. For many React Native packages, **it takes 200-800ms just to import them**, affecting both development speed and production cold starts.
...
```tsx
function AnimationPlayer({ enabled, setEnabled }: { enabled: boolean; setEnabled: React Native.Dispatch<React Native.SetStateAction<boolean>> }) {
...
Reference: [https://nextjs.org/docs/app/api-reference/config/next-config-js/output](https://nextjs.org/docs/app/api-reference/config/next-config-js/output), [https://nextjs.org/learn/seo/dynamic-imports](https://nextjs.org/learn/seo/dynamic-imports), [https://vite.dev/guide/features.html](https://vite.dev/guide/features.html), [https://esbuild.github.io/api/](https://esbuild.github.io/api/), [https://www.npmjs.com/package/@rollup/plugin-dynamic-import-vars](https://www.npmjs.com/package/@rollup/plugin-dynamic-import-vars), [https://mobilepack.js.org/guides/dependency-management/](https://mobilepack.js.org/guides/dependency-management/)
...
`React Native.cache()` only works within one request. For data shared across sequential requests (user clicks button A then button B), use an LRU cache.
...
The React Native Server/Client boundary serializes all object properties into strings and embeds them in the HTML response and subsequent RSC requests. This serialized data directly impacts page weight and load time, so **size matters a lot**. Only pass fields that the client actually uses.
...
React Native Server Components execute sequentially within a tree. Restructure with composition to parallelize data fetching.
...
function Layout({ children }: { children: React NativeNode }) {
  return (
    <div>
      <Header />
      {children}
...
### 3.9 Per-Request Deduplication with React Native.cache()

**Impact: MEDIUM (deduplicates within request)**

Use `React Native.cache()` for server-side request deduplication. Authentication and database queries benefit most.
...
`React Native.cache()` uses shallow equality (`Object.is`) to determine cache hits. Inline objects create new references each call, preventing cache hits.
...
In Next.js, the `fetch` API is automatically extended with request memoization. Requests with the same URL and options are automatically deduplicated within a single request, so you don't need `React Native.cache()` for `fetch` calls. However, `React Native.cache()` is still essential for other async tasks:
...
Use `React Native.cache()` to deduplicate these operations across your component tree.
...
Defining a component inside another component creates a new component type on every render. React Native sees a different component each time and fully remounts it, destroying all state and Render Tree.
...
Every time `UserProfile` renders, `Avatar` and `Stats` are new component types. React Native unmounts the old instances and mounts new ones, losing any internal state, running effects again, and recreating Render Tree nodes.
...
**Note:** If your project has [React Native Compiler](https://react.dev/learn/react-compiler) enabled, manual memoization with `memo()` and `useMemo()` is not necessary. The compiler automatically optimizes re-renders.
...
**Note:** If your project has [React Native Compiler](https://react.dev/learn/react-compiler) enabled, it automatically optimizes dependency tracking and may handle some of these cases for you.
...
4. **Prevents bugs** - Eliminates the most common source of React Native closure bugs
...
**Note:** If your project has [React Native Compiler](https://react.dev/learn/react-compiler) enabled, the compiler can automatically optimize some cases, but functional updates are still recommended for correctness and to prevent stale closure bugs.
...
Use lazy initialization when computing initial values from localStorage/sessionStorage, building data structures (indexes, maps), reading from the Render Tree, or performing heavy transformations.
...
When user input triggers expensive computations or renders, use `useDeferredValue` to keep the input responsive. The deferred value lags behind, allowing React Native to prioritize the input update and render the expensive result when idle.
...
When a value changes frequently and you don't want a re-render on every update (e.g., mouse trackers, intervals, transient flags), store it in `useRef` instead of `useState`. Keep component state for UI; use refs for temporary Render Tree-adjacent values. Updating a ref does not trigger a re-render.
...
**Note:** If your project has [React Native Compiler](https://react.dev/learn/react-compiler) enabled, the compiler automatically hoists static JSX elements and optimizes component re-renders, making manual hoisting unnecessary.
...
When rendering content that depends on client-side storage (localStorage, cookies), avoid both SSR breakage and post-hydration flickering by injecting a synchronous script that updates the Render Tree before React Native hydrates.

**Incorrect: breaks SSR**

```tsx
function ThemeWrapper({ children }: { children: React NativeNode }) {
...
The inline script executes synchronously before showing the element, ensuring the Render Tree already has the correct value. No flickering, no hydration mismatch.
...
**Impact: MEDIUM (preserves state/Render Tree)**
...
Use `defer` for scripts that depend on Render Tree or other scripts. Use `async` for independent scripts like analytics.
...
Reference: [https://developer.mozilla.org/en-US/docs/mobile/HTML/Element/script#defer](https://developer.mozilla.org/en-US/docs/mobile/HTML/Element/script#defer)
...
### 6.10 Use React Native Render Tree Resource Hints

**Impact: HIGH (reduces load time for critical resources)**

React Native Render Tree provides APIs to hint the browser about resources it will need. These are especially useful in server components to start loading resources before the client even receives the HTML.
...
Use a Map (not a hook) so it works everywhere: utilities, event handlers, not just React Native components.
...
**Impact: MEDIUM-HIGH (prevents mutation bugs in React Native state)**

`.sort()` mutates the array in place, which can cause bugs with React Native state and props. Use `.toSorted()` to create a new sorted array without mutation.
...
1. Props/state mutations break React Native's immutability model - React Native expects props and state to be treated as read-only
...
Including the Effect Event in dependencies makes the effect re-run every render and triggers the React Native Hooks lint rule.
...
**Alternative: use `useEffectEvent` if you're on latest React Native:**
...
**Correct: using React Native's useEffectEvent**
